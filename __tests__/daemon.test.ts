...
describe("validatePathWithinWorkspace", () => {
	it("accepts paths within workspace root", () => {
		expect(validatePathWithinWorkspace("data/tasks.json", "/workspace")).toBe(
			true,
		);
	});
	...
	it("rejects path traversal with ../", () => {
		expect(validatePathWithinWorkspace("../../etc/passwd", "/workspace")).toBe(
			false,
		);
	});
	...
	it("accepts nested subdirectories", () => {
		expect(
			validatePathWithinWorkspace("data/sub/file.json", "/workspace"),
		).toBe(true);
	});
});
...
import {
	...
	hasPendingDecision,
	isTaskUnblocked,
} from "../scripts/daemon/prompt-builder";
...
describe("getPendingTasks", () => {
	...
	it("returns tasks sorted by Eisenhower priority", () => {
		...
		for (let i = 0; i < tasks.length - 1; i++) {
			...
			const pB =
				priorityMap[`${tasks[i + 1].importance}-${tasks[i + 1].urgency}`] ?? 3;
