...
export async function POST(request: Request) {
	try {
		...
		// 1. Read the message
		const inboxData = readJSON<{ messages: InboxMessage[] }>(
			path.join(DATA_DIR, "inbox.json"),
		);
		if (!inboxData) {
			return NextResponse.json(
				{ error: "Could not read inbox" },
				{ status: 500 },
			);
		}
		...
		if (message.to === "me") {
			return NextResponse.json(
				{ error: "Cannot auto-respond for human user" },
				{ status: 400 },
			);
		}

		// 3. Verify the agent exists
		const agentsData = readJSON<{
		...
		const scriptPath = path.resolve(
			...
			"scripts",
			"daemon",
			"run-inbox-respond.ts",
		);

		try {
			const child = spawn(
				process.execPath,
				["--import", "tsx", scriptPath, messageId, "--run-id", runId],
				{
					cwd,
					detached: true,
					stdio: "ignore",
