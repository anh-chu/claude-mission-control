...
export default function InboxPage() {
	...
	interface RespondingInfo {
		runId: string;
		agent: string;
		since: number;
		sessionIndex: number;
	}
	const [respondingThreads, setRespondingThreads] = useState<
		Map<string, RespondingInfo>
	>(new Map());
					...
					for (const [threadKey, info] of prev) {
						...
						if (!serverRun) {
							...
							if (serverRun.continuationIndex !== info.sessionIndex) {
								next.set(threadKey, {
									...info,
									sessionIndex: serverRun.continuationIndex,
								});
							}
						}
					}
	...
	const handleMarkThreadRead = async (thread: Thread) => {
		...
		await Promise.all(
			unread.map((m) => updateMessage(m.id, { status: "read" as const })),
		);
	};
	...
	const handleArchiveThread = async (thread: Thread) => {
		...
		await Promise.all(
			nonArchived.map((m) =>
				updateMessage(m.id, { status: "archived" as const }),
			),
		);
	...
	const handleReply = (msg: InboxMessage) => {
		setComposeTo(
			msg.from === "me" ? (msg.to as AgentRole) : (msg.from as AgentRole),
		);
	...
	const triggerAutoRespond = useCallback(
		async (messageId: string, agentName: string, threadKey?: string) => {
			try {
				...
				if (res.ok) {
					...
					showErrorToast(
						`Auto-respond failed: ${data.error ?? "Unknown error"}`,
					);
				}
			} catch {
				showErrorToast(
					"Failed to trigger auto-respond — is the agent available?",
				);
			}
		},
		[],
	...
	const handleStopRespond = useCallback(
	...
	const handleSend = useCallback(async () => {
		...
		if (composeTo !== "me" && created) {
			const agentLabel =
				AGENT_ROLES.find((r) => r.id === composeTo)?.label ?? composeTo;
						...
						>
							<CardContent className="p-3">
								...
								<div className="flex items-center gap-3">
									...
									<Badge
										className={`text-xs shrink-0 ${typeColors[latestType]}`}
									>
										{latestType}
									</Badge>
									...
									<div className="flex items-center gap-2 shrink-0">
										...
										{isMulti && (
											<Tip
												content={`${thread.messages.length} messages in thread`}
											>
												<Badge
													variant="secondary"
									...
									<div className="mt-3 pt-3 border-t">
										...
										<div className="space-y-0">
											{thread.messages.map((msg, idx) => {
												const FromIcon =
													agentIcons[msg.from as AgentRole] ?? User;
												...
												return (
													...
													>
														<div className="flex items-center gap-2 mb-1">
															...
															<Badge
																className={`text-[10px] h-4 px-1 ${typeColors[msg.type]}`}
															>
																{msg.type}
															</Badge>
										...
										<div className="flex items-center gap-2 mt-3 pt-3 border-t">
											...
											{thread.latest.from === "me" &&
												thread.latest.to !== "me" && (
													<Tip
														content={`Ask ${AGENT_ROLES.find((r) => r.id === thread.latest.to)?.label ?? thread.latest.to} to compose a reply`}
													>
														<Button
															size="sm"
															variant="outline"
															...
															onClick={(e) => {
																...
																const agentLabel =
																	AGENT_ROLES.find(
																		(r) => r.id === thread.latest.to,
																	)?.label ?? thread.latest.to;
																triggerAutoRespond(
																	thread.latest.id,
																	agentLabel,
																	thread.key,
											...
											{thread.taskId && (
												<span className="text-xs text-muted-foreground ml-auto">
													Linked task:{" "}
													{tasks.find((t) => t.id === thread.taskId)?.title ??
														thread.taskId}
												</span>
											)}
										</div>
					...
					<div className="space-y-4">
						<div className="grid grid-cols-2 gap-3">
							<div className="space-y-1.5">
								...
								<Select
									value={composeTo}
									onValueChange={(v) => setComposeTo(v as AgentRole)}
								>
							...
							<div className="space-y-1.5">
								...
								<Select
									value={composeType}
									onValueChange={(v) => setComposeType(v as MessageType)}
								>
									<SelectTrigger className="h-8 text-xs">
										<SelectValue />
									</SelectTrigger>
