...
import { scrubCredentials, validateBinary } from "./security";
import type {
	AgentBackend,
	ClaudeOutputMeta,
	SpawnOptions,
...
function findClaudeBinary(): ResolvedBinary {
	...
	try {
		...
		if (config.execution.claudeBinaryPath) {
			logger.info(
				"runner",
				`Using configured binary path: ${config.execution.claudeBinaryPath}`,
			);
			cachedBinary = {
		...
		if (candidate && existsSync(candidate)) {
			...
			if (candidate.endsWith(".cmd")) {
				...
				if (jsEntry) {
					logger.info(
						"runner",
						`Resolved .cmd shim → ${jsEntry} (via node.exe)`,
					);
					cachedBinary = {
	...
	logger.warn(
		"runner",
		"Could not auto-detect claude binary. Set 'claudeBinaryPath' in daemon-config.json or install Claude Code globally (npm i -g @anthropic-ai/claude-code)",
	);
	return {
		bin: "claude",
		prefixArgs: [],
...
export class AgentRunner {
	...
	async spawnAgent(opts: SpawnOptions): Promise<SpawnResult & { pid: number }> {
		...
		if (!validateBinary(resolved.originalPath)) {
			throw new Error(
				`Security: binary "${resolved.originalPath}" is not in the allowed list`,
			);
		}
		...
		return new Promise<SpawnResult & { pid: number }>((resolve) => {
			...
			// Notify caller of PID immediately after spawn (for tracking in runs registry, etc.)
			opts.onSpawned?.(pid);
			...
			if (opts.streamFile) {
				try {
					...
					logger.warn(
						"runner",
						`Failed to create stream file ${opts.streamFile}: ${err instanceof Error ? err.message : String(err)}`,
					);
				}
			}
				...
				for (const line of lines) {
					...
					if (!sessionIdEmitted && opts.onSessionId && backend === "claude") {
						try {
							...
							/* not JSON or not the init event */
						}
					}
			...
			const timer = setTimeout(() => {
				...
				treeKill(pid, "SIGTERM", (err?: Error) => {
					if (err) {
						logger.error(
							"runner",
							`Failed to kill process tree ${pid}: ${err.message}`,
						);
						try {
							child.kill("SIGKILL");
						} catch {
			...
			child.on("error", (err: Error) => {
				...
				logger.error("runner", "Agent spawn error", {
					...
					binary: resolved.bin,
					error: err.message,
					hint: err.message.includes("ENOENT")
						? "Set claudeBinaryPath in daemon-config.json or install globally"
						: undefined,
				});
				resolve({
					pid,
		...
		return new Promise((resolve) => {
			treeKill(pid, "SIGTERM", (err?: Error) => {
				if (err) {
					logger.error(
						"runner",
						`Failed to kill session ${pid}: ${err.message}`,
					);
				} else {
