...
function DraggableTask({ task, onClick, isRunning, onRun, pendingDecisionTaskIds, onStatusChange, onDuplicate, onDelete }: { task: Task; onClick: () => void; isRunning?: boolean; onRun?: (taskId: string) => void; pendingDecisionTaskIds?: Set<string>; onStatusChange?: (taskId: string, status: KanbanStatus) => void; onDuplicate?: (task: Task) => void; onDelete?: (taskId: string) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} onPointerDown={handlePointerDown} {...attributes}>
      <TaskCard task={task} isDragging={isDragging} onClick={onClick} isRunning={isRunning} onRun={onRun} pendingDecisionTaskIds={pendingDecisionTaskIds} onStatusChange={onStatusChange} onDuplicate={onDuplicate} onDelete={onDelete} />
    </div>
  );
}

function DroppableZone({ id, label, dotColor, tasks, onTaskClick, children, isTaskRunning, onRunTask, pendingDecisionTaskIds, onStatusChange, onDuplicate, onDelete }: {
  ...
  onStatusChange?: (taskId: string, status: KanbanStatus) => void; onDuplicate?: (task: Task) => void; onDelete?: (taskId: string) => void;
}) {
  ...
  return (
    <div ref={setNodeRef} className={cn("flex flex-col rounded-xl border bg-card/50 min-h-[200px] transition-all", isOver && "ring-2 ring-primary/50")}>
      ...
      <div className="flex-1 space-y-2 p-2 overflow-y-auto max-h-[60vh]">
        ...
        {tasks.map((t) => <DraggableTask key={t.id} task={t} onClick={() => onTaskClick(t)} isRunning={isTaskRunning?.(t.id)} onRun={onRunTask} pendingDecisionTaskIds={pendingDecisionTaskIds} onStatusChange={onStatusChange} onDuplicate={onDuplicate} onDelete={onDelete} />)}
        {children}
      </div>
    </div>
  );
...
export function ProjectDetailPage({ parentLabel, parentHref }: ProjectDetailPageProps) {
  ...
  if (!project) {
    return (
      <div className="space-y-4">
        <BreadcrumbNav items={[{ label: parentLabel, href: parentHref }, { label: "Not Found" }]} />
  ...
  return (
    <div className="space-y-4">
      <BreadcrumbNav items={[{ label: parentLabel, href: parentHref }, { label: project.name }]} />
        ...
        <TabsContent value="priority-matrix">
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleEisenhowerDragEnd}>
            <div className="grid grid-cols-2 gap-3">
              <DroppableZone id="do" label="DO" dotColor="bg-quadrant-do" tasks={eGrouped.do} onTaskClick={setSelectedTask} isTaskRunning={isTaskRunning} onRunTask={runTask} pendingDecisionTaskIds={pendingDecisionTaskIds} onStatusChange={handleStatusChange} onDuplicate={handleDuplicate} onDelete={handleDeleteById} />
              <DroppableZone id="schedule" label="SCHEDULE" dotColor="bg-quadrant-schedule" tasks={eGrouped.schedule} onTaskClick={setSelectedTask} isTaskRunning={isTaskRunning} onRunTask={runTask} pendingDecisionTaskIds={pendingDecisionTaskIds} onStatusChange={handleStatusChange} onDuplicate={handleDuplicate} onDelete={handleDeleteById} />
              <DroppableZone id="delegate" label="DELEGATE" dotColor="bg-quadrant-delegate" tasks={eGrouped.delegate} onTaskClick={setSelectedTask} isTaskRunning={isTaskRunning} onRunTask={runTask} pendingDecisionTaskIds={pendingDecisionTaskIds} onStatusChange={handleStatusChange} onDuplicate={handleDuplicate} onDelete={handleDeleteById} />
              <DroppableZone id="eliminate" label="ELIMINATE" dotColor="bg-quadrant-eliminate" tasks={eGrouped.eliminate} onTaskClick={setSelectedTask} isTaskRunning={isTaskRunning} onRunTask={runTask} pendingDecisionTaskIds={pendingDecisionTaskIds} onStatusChange={handleStatusChange} onDuplicate={handleDuplicate} onDelete={handleDeleteById} />
            </div>
        ...
        <TabsContent value="status-board">
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleKanbanDragEnd}>
            <div className="grid grid-cols-3 gap-3">
              <DroppableZone id="not-started" label="Not Started" dotColor="bg-status-not-started" tasks={kGrouped["not-started"]} onTaskClick={setSelectedTask} isTaskRunning={isTaskRunning} onRunTask={runTask} pendingDecisionTaskIds={pendingDecisionTaskIds} onStatusChange={handleStatusChange} onDuplicate={handleDuplicate} onDelete={handleDeleteById} />
              <DroppableZone id="in-progress" label="In Progress" dotColor="bg-status-in-progress" tasks={kGrouped["in-progress"]} onTaskClick={setSelectedTask} isTaskRunning={isTaskRunning} onRunTask={runTask} pendingDecisionTaskIds={pendingDecisionTaskIds} onStatusChange={handleStatusChange} onDuplicate={handleDuplicate} onDelete={handleDeleteById} />
              <DroppableZone id="done" label="Done" dotColor="bg-status-done" tasks={kGrouped.done} onTaskClick={setSelectedTask} isTaskRunning={isTaskRunning} onRunTask={runTask} pendingDecisionTaskIds={pendingDecisionTaskIds} onStatusChange={handleStatusChange} onDuplicate={handleDuplicate} onDelete={handleDeleteById} />
            </div>
            <DragOverlay>{activeTask ? <TaskCard task={activeTask} className="shadow-xl" /> : null}</DragOverlay>
          </DndContext>
        </TabsContent>
        ...
        <TabsContent value="milestones">
          {longTermGoals.length === 0 && milestones.length === 0 ? (
            ...
            <div className="space-y-3">
              {longTermGoals.map((g) => (
                <GoalCard key={g.id} goal={g} tasks={tasks} projects={projects} milestones={milestones} onMarkComplete={handleMarkGoalComplete} onDelete={handleDeleteGoal} />
              ))}
              {milestones.filter((m) => !m.parentGoalId).map((m) => (
                <GoalCard key={m.id} goal={m} tasks={tasks} projects={projects} milestones={[]} onMarkComplete={handleMarkGoalComplete} onDelete={handleDeleteGoal} />
              ))}
            </div>
          )}
        </TabsContent>
