...
export function TaskContextMenuContent({
  ...
  return (
    <ContextMenuContent>
          ...
          <ContextMenuSub>
            ...
            <ContextMenuSubContent>
              {task.kanban !== "not-started" && (
                <ContextMenuItem onClick={() => onStatusChange(task.id, "not-started")}>
                  Not Started
                </ContextMenuItem>
              )}
              {task.kanban !== "in-progress" && (
                <ContextMenuItem onClick={() => onStatusChange(task.id, "in-progress")}>
                  In Progress
                </ContextMenuItem>
              )}
              {task.kanban !== "done" && (
                <ContextMenuItem onClick={() => onStatusChange(task.id, "done")}>
                  Done
                </ContextMenuItem>
              )}
            </ContextMenuSubContent>
      ...
      {onDuplicate && (
        <ContextMenuItem onClick={() => onDuplicate(task)}>Duplicate</ContextMenuItem>
      )}
